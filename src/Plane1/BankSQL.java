...
public class BankSQL {
    ...
    public void bJoin(Plane1.Client client) {
    ...
    public void bCreate(Client client) {
        String sql = "INSERT INTO BANKTEAM VALUES (MADMIN,?,?)";
        try {
            pstmt = con.prepareStatement(sql);
            pstmt.setString(1, client.gettAccount());
            pstmt.setInt(2,client.getcCode());
    ...
    public void select(Client client) {
        String sql = "SELECT b.CNAME, c.MNAME, a.TACCOUNT, a.TBALANCE " +
...
//            pstmt.executeUpdate();
    ...
    public void pSelect(Client cl) {
        String sql = "SELECT TNAME,TACCOUNT FROM BANKTEAM WHERE TNAME=?";
        try {
            pstmt = con.prepareStatement(sql);
            pstmt.setString(1, cl.getmName());
//            pstmt.executeUpdate();
            rs = pstmt.executeQuery();
            while (rs.next()) {
    ...
    public void bPCreate(Client cl) {
        String sql = "INSERT INTO BANKTEAM VALUES ('admin',?,0,1,?)";
        try {
            pstmt = con.prepareStatement(sql);
            pstmt.setString(1, cl.gettAccount());
            pstmt.setString(2, cl.getmName());
            int result = pstmt.executeUpdate();
            if (result > 0) {
                System.out.println("계좌가 생성되었습니다!");
            } else {
